...
timer_sleep (int64_t ticks)
{
  ...
  /* Calculate the wake up time for current thread and block it.
     Wakeup time(in ticks) = current time(in ticks) + sleep time(in ticks) */
  int64_t start = timer_ticks ();
  int64_t wake = start + ticks;
  struct thread *current = thread_current ();
  current->waketime = wake;

  thread_block();  /* Push the thread to waiting queue.*/
  intr_set_level (old_level);
}
